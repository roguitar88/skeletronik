...
class ModelManageDB extends ModelConnection
{
    public function updateCurrentDB($db, $dbms_schema)
    {
        ...
        *
        ...
        try {
            // $drop_db = $this->pdo()->prepare("DROP database zuump");
            // $drop_db->execute();

            // $create_db = $this->pdo()->prepare("CREATE database `zuump` /*!40100 COLLATE 'utf8mb4_unicode_ci' */");
            // $create_db->execute();

            // https://stackoverflow.com/questions/147821/loading-sql-files-from-within-php
            $new_db = file_get_contents($dbms_schema);
            $this->pdo($db)->exec($new_db);
            ...
            */
            ...
            // die("ERROR: Could not able to execute MySQL command. ". $e->getMessage());
            return array(
                "success" => false,
                "error" => "ERROR: Could not be able to execute MySQL command. ". $e->getMessage()
            );
        }
    }
